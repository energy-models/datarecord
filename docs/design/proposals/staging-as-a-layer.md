...
`staged_only()` is the half still open, and is now its own proposal: [`staged_only()` is the staged source](staged-only-is-the-source.md). The duplication is exact rather than approximate — each of its four builders calls the same thing the matching `StagedSource` member does — and what it needs first is the listing members `LayerSource` lacks, the fold never having had to list a key set it reads off the owner map.
