...
- **`Dimension`** declares one axis: its `dtype`, whether it `keys` the entity tables (a component exists _per scenario_, [design](../design/schema.md#keys-which-entity-tables-a-dim-keys)), and `within` for an axis whose labels identify a point only inside another's — multi-period time being the case ([design](../design/schema.md#within-an-axis-inside-an-axis)).
