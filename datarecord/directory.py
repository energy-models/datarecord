...
class DirectoryRecord:
    ...
    def _by_type(self, subdir: str) -> LazyFrames:
        ...
        return LazyFrames(
            types, lambda ctype: nw.from_native(self._member_frame(subdir, ctype))
        )

    def _member_frame(self, subdir: str, ctype: str) -> DuckDBPyRelation:
        """One type's members as the owner map presents them.

        A written layer's fold bookkeeping is dropped, and `order_key`
        regenerated by scan order, the order the map derives it from too.
        """
        rel = self._require(f"{subdir}/{ctype}.parquet")
        fold = ("component_type", "layer_uuid", "deleted", "order_key")
        return rel.project(
            star(exclude=[c for c in fold if c in rel.columns]),
            sql("row_number() OVER ()").alias("order_key"),
        )
